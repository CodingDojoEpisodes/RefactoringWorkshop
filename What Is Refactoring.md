...
1. N: So, if I understand correctly, the majority of the time is used for writing the software and just a small portion of the effort is about refactoring. Why investing in building a skill that is only rarly used?
1. I: Oh, you got it all wrong, I should have said it at the beginning. Refactoring is not a task on the task bord. Refacatoring is an ongoing activity. Infact, I spend more time refactoring the existing code than writing new code. The developmnet of almoste every new feature starts with refactoring in order to make the neccessary adjustments to the current design so that the new code will be elegantly introducesd into the existing solution.
